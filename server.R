...
### Fonction 8.1 "NPS Simple"
plot_bivariate_NPS <- function(data, variable, group_var, afficher_graph = TRUE) {
  library(dplyr)
  library(stringr)
  library(plotly)
  library(tidyr)
  ...
  # Titre intelligent
  brackets_content <- str_extract_all(variable, "\\[.*?\\]")[[1]]
  if (length(brackets_content) >= 2) {
    titre_question <- brackets_content[2] %>% str_remove_all("\\[|\\]")
  } else {
    titre_question <- variable %>%
      str_replace(".*?\\]\\.\\s*", "") %>%
      str_extract("^[^\\[]+") %>%
      str_trim()
  }
  titre_question_wrapped <- str_wrap(titre_question, width = 80)
  ...
  df <- data %>%
    ...
    mutate(
      Note = as.numeric(.data[[variable]]),
      Groupe = str_to_title(str_trim(as.character(.data[[group_var]])))
    ) %>%
    filter(!is.na(Note))
  
  if (nrow(df) == 0) stop("❌ Données insuffisantes après nettoyage.")
  
  # Appliquer ordre si reconnu
  modalites <- unique(df$Groupe)
  if (all(tolower(modalites) %in% ordre_personnalise_lower)) {
    df$Groupe <- factor(df$Groupe, levels = ordre_personnalise)
  } else {
    df$Groupe <- factor(df$Groupe, levels = sort(modalites))
  }
  
  # 🏷️ Catégorisation NPS
  if (all(df$Note %in% 0:5)) {
    df <- df %>%
  ...
  # Résumé
  df_summary <- df %>%
    group_by(Groupe, NPS_type) %>%
    summarise(n = n(), .groups = "drop") %>%
    group_by(Groupe) %>%
    mutate(pct = 100 * n / sum(n)) %>%
    ungroup()
  
  # Calcul NPS par groupe
  nps_df <- df_summary %>%
    filter(NPS_type %in% c("Promoteur", "Détracteur")) %>%
    pivot_wider(names_from = NPS_type, values_from = pct, values_fill = 0) %>%
    mutate(NPS = Promoteur - Détracteur)
  
  # Couleurs
  couleurs <- c("Détracteur" = "#E74C3C", "Passif" = "#F1C40F", "Promoteur" = "#2ECC71")
  
  if (afficher_graph) {
    df_summary <- df_summary %>%
      mutate(
        pourcentage_text = paste0(round(pct, 1), "%"),
        hover_label = paste0(NPS_type, "<br>", round(pct, 1), "%<br>Effectif : ", n)
      )
    ...
    fig <- plot_ly(
      ...
      type = "bar",
      colors = couleurs,
      text = ~pourcentage_text,       # Affichage uniquement du %
      textposition = "auto",
      textfont = list(color = "black"),
      hoverinfo = "text",
      hovertext = ~hover_label        # Hover : affiche % + effectif
    ) %>%
      layout(
        barmode = "stack",            # Barres empilées
        yaxis = list(title = "Pourcentage", ticksuffix = "%", range = c(0, 100)),
        xaxis = list(title = str_wrap(group_var, 80)),
        showlegend = TRUE,
        margin = list(t = 120, b = 80),
        annotations = list(
          list(
            text = titre_question_wrapped,
            x = 0.5, y = 1.12, xref = "paper", yref = "paper",
            showarrow = FALSE, font = list(size = 14, color = "#2C3E50"),
            align = "center"
          )
        )
      )
...
### Fonction 8.2 "Multi-choix"

# Fonction principale NPS avec gestion couleurs colorspace
plot_bivariate_NPS_by_code <- function(data, code_prefix, group_var, titre = NULL) {
  ...
  gg <- ggplot(nps_results, aes(x = Ressource, y = NPS, fill = group_var_clean, text = tooltip)) +
    ...
    geom_text(aes(label = sprintf("%.2f", NPS)),
              position = position_dodge(width = 0.9),
    ...
    labs(
      title = titre,
      x = "", 
      y = "Taux de satisfaction/préférence",
      fill = str_replace_all(group_var, "(.{1,30})(\\s)", "\\1\n")
    ) +
    theme_minimal() +
    theme(
...
plot_bivariate_classement <- function(data, code_prefix, var_group, titre = NULL, stacked = FALSE) {
  ...
  pos_geom_text <- if (stacked) position_stack(vjust = 0.5) else position_dodge(width = 0.8)
  pos_geom_col <- if (stacked) "stack" else "dodge"
  
  hjust_geom_text <- if (stacked) -0.1 else 0.5
...
server <- function(input, output, session) {
  ...
  lapply(1:5, function(i) {
    output[[paste0("bivar_quanti_plot_", i)]] <- renderPlotly({
      ...
      if (is_nps) {
        # Détection des colonnes correspondant au code_prefix (hors Autres et Commentaires)
        cols_found <- names(df) %>%
        ...
        is_modalite_choisie <- !is.null(modalite) && modalite != ""
        
        if (mode_selected == "classement") {
          stacked_val <- is_multi_modalite && !is_modalite_choisie
          plot_bivariate_classement(
            data = df,
            code_prefix = if (is_modalite_choisie) modalite else prefix,
            var_group = group_var,
            stacked = stacked_val
          )
        } else {
          if (is_multi_modalite && !is_modalite_choisie) {
            # Appel version multi-modalité
            plot_bivariate_NPS_by_code(
              data = df,
              code_prefix = prefix,
              group_var = group_var
            )
          } else {
            # Appel version simple NPS
            plot_bivariate_NPS(
              data = df,
              var = if (is_modalite_choisie) modalite else cols_found[1],
              group_var = group_var
            )
          }
        }
